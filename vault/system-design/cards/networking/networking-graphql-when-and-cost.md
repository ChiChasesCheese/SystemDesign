...
什么客户端情况使 GraphQL 值得其复杂性，它导入什么两个运维问题？

当**许多不同的客户端需要同一图的不同切片**（移动 vs 网页 vs 合作伙伴）时值得 — 客户端查询他们需要的正好字段，消除过度/不足获取和每个客户端后端端点（BFF）。

成本：
- **缓存变得困难**：一切都是 POST 到一个端点，所以 HTTP/CDN 缓存不再免费工作。
- **无限制查询成本**：客户端可以编写病态嵌套查询 — 你必须添加深度/复杂度限制并用 dataloaders 解决 N+1。

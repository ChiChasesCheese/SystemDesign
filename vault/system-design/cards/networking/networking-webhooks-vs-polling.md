...
将事件暴露给第三方集成器：webhook vs 让他们轮询。webhook 提供者必须构建什么，轮询免费提供？

交付机制，因为消费者端点持续关闭或缓慢：

- **带回退的重试 + 死信处理** — 由于重试重新排序和复制，消费者需要事件 id 和幂等处理器。
- **推送的身份验证**：HMAC-signed 有效负载，使接收者可以验证源。
- **反正还有重放/列表 API** — 消费者必须在缺少交付后恢复。

轮询更简单且消费者节奏，但延迟 = 轮询间隔，大多数轮询返回无。强大模式：webhook 作为"某物改变"ping；消费者通过 API 获取真相。

...
一个挂起的长轮询请求对服务器成本什么，为什么每个线程的请求服务器首先达到上限？

每个等待的客户端持有一个开放连接和一个停泊请求长达轮询超时（保持在约 30 秒，在代理空闲限制下）。使用每个线程请求服务器这是一个线程 + 堆栈每个*空闲*客户端 — 并发上限在数千；事件循环/异步服务器改为持有一个廉价文件描述符并达到 100k+。

第二个成本：广播事件一次完成每个停泊的请求，所有这些客户端**立即重新轮询** — 每个事件后的同步请求波，SSE/WebSocket 通过保持通道开放来避免。

...
L4 vs L7 负载均衡器 — 每个看到什么，什么时候 L4 是正确选择尽管 L7 的灵活性？

- **L4** 只看到 IP+port：转发 TCP/UDP 流，无负载检查。极快、数百万连接、协议不可知。
- **L7** 终止连接并看到请求：路径/头路由、TLS 终止、重试、每路由策略。

为原始吞吐量或非 HTTP 流量（数据库、MQTT、游戏服务器、WebSocket 大规模直通）选择**L4** — 或作为**在**L7 舰队前面的弹性层，这是常见的堆叠模式。

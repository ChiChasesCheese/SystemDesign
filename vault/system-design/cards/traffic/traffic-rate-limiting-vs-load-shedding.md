...
速率限制 vs 负载削减 — 不同的触发器、不同的公平性。绘制区别并说为什么你需要两者。

- **速率限制**：*每客户端契约*（配额），即使在 10% 利用率下也执行 — 保护租户彼此并使成本可预测。
- **负载削减**：*在实际过载下的自我保护*，由健康信号（队列深度、CPU、延迟）触发，无论任何人的配额如何都丢弃流量 — 保护服务本身。

削减是优先级排序的：在付费交互请求之前丢弃未认证、后台和重试流量；永远不削减健康检查。

你需要两者，因为每个客户端保持在配额内仍然可以总和超过系统能够服务的。

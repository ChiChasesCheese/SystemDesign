...
反向代理请求/响应缓冲 — 它保护上游免受什么，何时必须关闭它？

代理完整吸收缓慢客户端的上传，然后以 LAN 速度转发到上游；响应亦然：上游在毫秒内交付完整响应，代理逐滴送出。上游工作者停止被缓慢或恶意客户端质押（slowloris 风格连接耗尽）。

当增量交付*是*产品时关闭：

- **SSE / 流响应** — 缓冲保持事件直到响应完成（`X-Accel-Buffering: no` / `proxy_buffering off`）。
- **大型上传** — 缓冲将它们加倍到代理磁盘/内存。
- WebSocket 升级后无论如何绕过缓冲。

...
请求对冲：机制、使其便宜的成本数学、其前提是什么？

如果在大约**p95 延迟**内没有回复到达，将相同的请求发送到第二个副本；取任何先答复的，取消另一个。用户的尾巴变成两个抽取的*最小值* — 尾部事件（GC 暂停、缓慢磁盘）很少同时打击两个副本。

成本数学：只在 p95 后对冲意味着 ≤5% 的请求被复制 — 约 5% 额外负载为 p99 购买数量级更好。

前提：幂等（或去重）操作、跨副本取消、永远不对冲对冲 — 否则过载将其变成自放大风暴。

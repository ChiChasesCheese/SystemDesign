...
负载均衡器本身是单点故障。LB 层如何高度可用，什么健康检查微妙性防止它使故障更糟？

HA：**共享虚拟 IP 的冗余 LB 对**（VRRP/keepalived 故障转移），或**anycast/ECMP** 将一个 IP 分布在 LB 舰队；DNS 作为粗外层有多个记录。

健康检查微妙性：区分**"这个实例宕机"与"所有东西都宕机"。** 如果所有后端未通过检查（例如共享依赖突发），删除它们全部服务 100% 错误 — 使用故障开放阈值（"如果 >50% 不健康，继续路由到所有"）和测试过程的检查，而不是其依赖。

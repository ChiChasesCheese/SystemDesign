---
nodes: [foundations]
url: https://github.com/donnemartin/system-design-primer
tags: [intro, reference]
---
# The System Design Primer (Donne Martin)

The most-starred open index of system-design fundamentals — trade-off axes
(performance vs scalability, latency vs throughput, CAP), the interview
approach, and back-of-envelope tables, all in one repo with sources linked.

**Extract on read:**
- The step-by-step interview approach: use cases → constraints → high-level → core → scale.
- The trade-off framings in "Performance vs scalability" and "Latency vs throughput".
- The "Powers of two" and per-component tables for estimation drills.

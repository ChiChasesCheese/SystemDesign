...
def _project(tmp_path):
    ...
    (tmp_path / "r.md").write_text(READING, encoding="utf-8")
    clips = tmp_path / "clippings"
    clips.mkdir()
    return (
        load_skeleton(tmp_path / "s.yaml"),
        [parse_reading(tmp_path / "r.md")],
        clips,
    )
...
def test_go_deeper_prefers_the_local_clipping(tmp_path):
    skeleton, readings, clips = _project(tmp_path)

    # no clipping yet -> plain web link
    links = go_deeper(skeleton, readings, "alpha.one", {}, "myvault", tmp_path)
    assert links[0].href.startswith("https://") and not links[0].is_local

    (clips / "alpha-guide.md").write_text(WEB_CLIPPER_NOTE, encoding="utf-8")
    links = go_deeper(
        skeleton, readings, "alpha.one", load_clippings(clips), "myvault", tmp_path
    )
    assert links[0].is_local
    assert links[0].href == open_uri("myvault", "clippings/alpha-guide")
...
def test_pdf_clipping_is_embedded_not_converted(tmp_path):
    ...
    path = write_clipping(tmp_path, "raft-paper", "https://x.com/raft.pdf", page,
                          "2026-08-15")
    assert (tmp_path / "raft-paper.pdf").read_bytes() == b"%PDF-1.4 fake"
    assert "![[raft-paper.pdf]]" in path.read_text(encoding="utf-8")
...
def test_built_card_carries_the_obsidian_link_and_a_web_fallback(tmp_path):
    ...
    skeleton, readings, clips = _project(tmp_path)
    write_clipping(clips, "alpha-guide", "https://example.com/Guide",
    ...
    build_package(skeleton, [parse_card(card_file)], out, readings,
                  clippings=load_clippings(clips), vault="vault",
                  vault_root=tmp_path)
    ...
    assert 'href="obsidian://open?vault=vault&file=clippings%2Falpha-guide"' in flds
